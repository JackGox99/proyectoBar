...
// List retorna los usuarios, opcionalmente ordenados por el query param `sort_by`.
// Valores soportados: "username" (QuickSort), "nombre"/"name" (MergeSort).
func (uc *UserController) List(c *gin.Context) {
	sortBy := c.Query("sort_by")
	users, err := uc.service.List(sortBy)
	if err != nil {
...
// Deactivate implementa HU011 — User Deactivation (eliminación lógica).
// Acceso restringido a admin (aplicado vía middleware RequireRole en routes.go).
func (uc *UserController) Deactivate(c *gin.Context) {
	id, err := strconv.ParseUint(c.Param("id"), 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid id"})
		return
	}
	if err := uc.service.Deactivate(uint(id)); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
